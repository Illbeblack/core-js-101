...
function findFirstSingleChar(str) {
  return str.split('').filter((val) => (str.indexOf(val) === str.lastIndexOf(val) ? val : null))[0] || null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const startChar = isStartIncluded ? '[' : '(';
  const endChar = isEndIncluded ? ']' : ')';
  return `${startChar}${start}, ${end}${endChar}`;
}
...
function isCreditCardNumber(ccn) {
  const numString = String(ccn);
  const numDig = numString.length;
  let sum = 0;
  const parity = (numDig - 2) % 2;
  for (let i = 0; i < numDig; i += 1) {
